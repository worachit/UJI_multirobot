...
private:
	...
	static inline bool isConstraint(int agentID, int x, int y, int time, const std::vector<Constraint2> &constraints);
...
public:
	...
	std::vector<Cell> solve(const std::vector<Constraint2> &constraints, const Map &map, int agentID);
	std::vector<std::vector<Cell>> findOptimalPaths(const std::vector<Constraint2> &constraints, const Map &map);
