...
Conflict2 HighLevelSolver::getFirstConflict(const TreeNode &P) {
		...
		for (int j = i + 1; j < solutions.size(); j++) {
			...
			for (int k = 0; k < min_index; k++) {
				if (solutions[i][k] == solutions[j][k])
					return Conflict2(i, j, solutions[i][k], solutions[i][k], k);
			}
		}
	}
		...
		for (int j = i + 1; j < solutions.size(); j++) {
			...
			for (int k = 0; k < min_index; k++) {
				...
				if (route1[k] == route2[k + 1] && route1[k + 1] == route2[k])
					return Conflict2(i, j, route1[k + 1], route2[k + 1], k);
			}
		}
	}
	// Trivial. It is assured that this method is called only when there is conflict
	return Conflict2(0, 0, Cell(0, 0), Cell(0, 0), 0);
}
	...
	while (!isEmpty(tree)) {
		...
		return P.getSolution();
            ...
            for (int i = 0; i < 2; i++) {
                ...
                auto newConstraint = Constraint2(conflict.conflictedAgentsID.first, conflict.cell1, conflict.time);

                if (i == 1) {
                    newConstraint = Constraint2(conflict.conflictedAgentsID.second, conflict.cell2, conflict.time);
                }
            ...
            for (int i = 0; i < 2; i++) {
                ...
                auto newConstraint = Constraint2(conflict.conflictedAgentsID.first, conflict.cell1, conflict.time - 1);
                auto constrain2 = Constraint2(conflict.conflictedAgentsID.first, conflict.cell2, conflict.time);
                if (i == 1) {
                    newConstraint = Constraint2(conflict.conflictedAgentsID.second, conflict.cell2, conflict.time - 1);
                    constrain2 = Constraint2(conflict.conflictedAgentsID.second, conflict.cell1, conflict.time);
                }
